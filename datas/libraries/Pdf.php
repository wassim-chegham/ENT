...
require ('../../../tools/php/pdf/pdf_gen/fpdf.class.php');
...
class Pdf extends FPDF
{
    ...
    function insertTOC($location = 1, $labelSize = 20, $entrySize = 10, $tocfont = 'Times', $label = 'Table des mati�res')
    {
        //make toc at end 
        $this->stopPageNums();
        $this->AddPage();
    ...
    function Set_Table_Type($type_arr)
    {
        ...
        if (!isset($type_arr['L_MARGIN']))
            $type_arr['L_MARGIN'] = 0;//default values 
    ...
    function Draw_Table_Border()
    {
        ...
        if (!$this->tb_border_draw)
            return;
            
        if (!$this->Data_On_Current_Page)
        ...
        //draw the border 
        $this->Rect($this->table_startx, $this->table_starty, $this->Get_Table_Width(), $this->GetY() - $this->table_starty);
    ...
    function Draw_Header_($next_line_height = 0)
    {
        ...
        for ($i = 0; $i < $this->tb_columns; $i++)
        {
            ...
            $this->SetFont($this->tb_header_type[$i]['T_FONT'], $this->tb_header_type[$i]['T_TYPE'], $this->tb_header_type[$i]['T_SIZE']);
            ...
            $this->tb_header_type[$i]['CELL_LINES'] = $this->NbLines($this->tb_header_type[$i]['CELL_WIDTH'], $this->tb_header_type[$i]['TEXT']);
        ...
        for ($i = 0; $i < $this->tb_columns; $i++)
        {
            ...
            //Print the text 
            $this->MultiCellTable($this->tb_header_type[$i]['CELL_WIDTH'], $this->tb_header_type[$i]['LN_SIZE'], $this->tb_header_type[$i]['TEXT'], $this->tb_header_type[$i]['BRD_TYPE'], $this->tb_header_type[$i]['T_ALIGN'], $this->tb_header_type[$i]['V_ALIGN'], 1, $h - $this->tb_header_type[$i]['LN_SIZE'] * $this->tb_header_type[$i]['CELL_LINES']);
    ...
    function Draw_Data($data, $header = true)
    {
        ...
        for ($i = 0; $i < $this->tb_columns; $i++)
        {
            ...
            if (!isset($data[$i]['V_ALIGN']))
                $data[$i]['V_ALIGN'] = $this->tb_data_type[$i]['V_ALIGN'];
            ...
            $this->SetFont($data[$i]['T_FONT'], $data[$i]['T_TYPE'], $data[$i]['T_SIZE']);
        ...
        for ($i = 0; $i < $this->tb_columns; $i++)
        {
            ...
            //print the text 
            $this->MultiCellTable($data[$i]['CELL_WIDTH'], $data[$i]['LN_SIZE'], $data[$i]['TEXT'], $data[$i]['BRD_TYPE'], $data[$i]['T_ALIGN'], $data[$i]['V_ALIGN'], 1, $h - $data[$i]['LN_SIZE'] * $data[$i]['CELL_LINES']);
    ...
    function CheckPageBreak($h, $header = true)
    {
        ...
        if ($this->GetY() + $h > $this->PageBreakTrigger)
        {
            ...
            $this->table_starty = $this->GetY();
            if ($header)
                $this->Draw_Header();//if we have to draw the header!!! 
        }
    ...
    function drawTableau(&$pdf, $tableType, $headerType, $headerDatas, $datasType, $datas)
    {
        ...
        for ($i = 0; $i < $nbCol; $i++)
        {
            ...
            {
                //echo "balise = " . $balise; 
                if ((strpos($balise, "I") === FALSE) && (strpos($balise, "B") === FALSE) && (strpos($balise, "U") === FALSE) && (strpos($balise, "L") === FALSE) && (strpos($balise, "C") === FALSE) && (strpos($balise, "R") === FALSE)) 
                ...
                else
                {
                    ...
                    if (strpos($balise, "I") === FALSE) 
                        ...
                        $header_type[$i]['T_TYPE'] .= 'I';
                    if (strpos($balise, "B") === FALSE) 
                        ;
                    else 
                        $header_type[$i]['T_TYPE'] .= 'B';
                    if (strpos($balise, "U") === FALSE) 
                        ;
                    else 
                        $header_type[$i]['T_TYPE'] .= 'U';
                    if (strpos($balise, "L") === FALSE) 
                        ;
                    else 
                        $header_type[$i]['T_ALIGN'] .= 'L';
